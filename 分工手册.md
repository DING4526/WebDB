...
负责基于地理位置展示抗战事件，提供空间维度的历史视角。主要工作集中在首页的交互式地图开发。

*   **核心代码文件**：
    *   `frontend/controllers/EventController.php`: 地图数据接口。
        *   `actionIndex`: 实现了 `get-active-locations` AJAX 接口，返回按地点分组的事件数据（含图片路径处理）。
    *   `frontend/views/site/index.php`: 首页地图容器。
        *   嵌入 `china-map.svg` 矢量地图。
        *   构建事件详情弹窗（集成 Swiper 轮播图结构）。
    *   `frontend/web/js/china-map.js`: 地图核心交互逻辑。
        *   实现 SVG 地图的加载与省份名称映射。
        *   **动态绘制红旗动画**：在有事件的省份位置绘制飘动的红旗。
        *   实现点击交互：点击省份/红旗弹出模态框，展示该地点的抗战历史事件。

*   **主要贡献**：
    *   实现了基于 SVG 的高性能交互地图，无需依赖庞大的第三方地图库。
    *   开发了红旗飘动动画效果，增强页面视觉感染力。
    *   实现了“地图-弹窗”的无刷新浏览体验，支持多图轮播查看。
...
以下目录树展示了项目的核心文件结构，**加粗**部分为各成员重点维护的代码文件。

```text
d:\xampp\htdocs\advanced\
├── common/
│   ├── config/
│   │   └── main.php                # 全局配置 (数据库、语言、时区)
│   └── models/                     # [核心] 数据模型层 (Ding)
│       ├── WarEvent.php            # 事件模型 (关联人物、阶段)
│       ├── WarPerson.php           # 人物模型 (关联事件、媒体)
│       ├── WarMessage.php          # 留言模型 (含审核逻辑)
│       ├── WarVisitLog.php         # 访问日志模型
│       ├── WarMedia.php            # 多媒体资源模型
│       ├── WarStage.php            # 战争阶段模型
│       ├── WarEventPerson.php      # 事件-人物关联模型
│       ├── User.php                # [系统] 用户模型
│       ├── LoginForm.php           # [系统] 登录表单模型
│       ├── Team.php                # [后台] 团队管理模型
│       ├── TeamMember.php          # [后台] 成员管理模型
│       ├── TeamMemberApply.php     # [后台] 成员申请模型
│       ├── Event.php               # (已废弃) 旧事件模型，请使用 WarEvent
│       └── Tag.php                 # (已废弃) 旧标签模型
│
├── frontend/                       # [核心] 前台展示端
│   ├── controllers/                # 控制器层
│   │   ├── EventController.php     # [地图] 地图数据接口 (孔祥昊)
│   │   ├── MessageController.php   # [留言] 留言提交逻辑 (Ding)
│   │   ├── PersonController.php    # [人物] 人物库逻辑 (苏奕扬)
│   │   ├── TimelineController.php  # [时间轴] 时间轴逻辑 (liyu)
│   │   └── SiteController.php      # 首页与静态页
│   │
│   ├── views/                      # 视图层
│   │   ├── layouts/
│   │   │   └── main.php            # 全局主布局 (导航栏、页脚)
│   │   ├── site/
│   │   │   └── index.php           # [地图] 首页地图容器 (孔祥昊)
│   │   ├── timeline/
│   │   │   ├── index.php           # [时间轴] 垂直时间轴列表 (liyu)
│   │   │   └── view.php            # [时间轴] 事件详情页 (liyu)
│   │   ├── person/
│   │   │   ├── index.php           # [人物] 人物卡片墙 (苏奕扬)
│   │   │   └── view.php            # [人物] 人物详情页 (苏奕扬)
│   │   └── message/
│   │       └── index.php           # [留言] 留言板页面 (Ding)
│   │
│   └── web/                        # 静态资源
│       ├── css/
│       │   ├── site.css            # 全局样式
│       │   └── timeline-style.css  # [时间轴] 专用样式 (liyu)
│       ├── js/
│       │   └── china-map.js        # [地图] SVG交互逻辑 (孔祥昊)
│       └── images/                 # 图片资源 (china-map.svg 等)
│
└── backend/                        # [核心] 后台管理端 (Ding)
    ├── controllers/                # 管理控制器 (Person, Event, Media CRUD)
    └── views/                      # 管理视图 (集成 AbsoluteAdmin 模板)
```
